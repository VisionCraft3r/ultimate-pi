 ...
 *   PI_BROWSER_PROFILE     override the user-data dir (default
 *                          <agentDir>/extensions/browser/.profile, where
...
export function resolveBrowserProfileDir(): string {
  return (
    ...
    join(getAgentDir(), "extensions", "browser", ".profile")
  );
}
...
export default function browserExtension(pi: ExtensionAPI) {
  ...
  if (typeof register === "function") {
    for (const name of BROWSER_SUBAGENT_TOOLS) {
      register(name, THIS_FILE);
    }
  }
