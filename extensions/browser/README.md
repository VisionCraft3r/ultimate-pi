...
- Browser state (cookies, localStorage, IndexedDB) is persisted to
  `<agentDir>/extensions/browser/.profile` (where `agentDir` is
  `PI_CODING_AGENT_DIR` or `~/.pi/agent`) via
...
| `PI_BROWSER_PROFILE` | `<agentDir>/extensions/browser/.profile` | Override the persistent user-data dir. Set to a tempdir for ephemeral sessions. |
...
- `playwright-core` ships without browser binaries; the `npx playwright-core install
